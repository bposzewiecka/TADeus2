...
class Phenotype(models.Model):
    ...
    def url(self): 
        ...
        if self.db  == 'HPO':
            url = 'http://compbio.charite.de/hpoweb/showterm?id={pheno_id}'.format(pheno_id = self.pheno_id)

        if self.db  == 'OMIM':
            url = 'https://www.omim.org/entry/{pheno_id}'.format(pheno_id = self.pheno_id)
